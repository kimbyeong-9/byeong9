...
import SavedScheduleCard from '../components/profile/SavedScheduleCard';
import PostListItem from '../components/profile/PostListItem';
...
const UserProfile = () => {
  ...
  const handleFollowClick = () => {
    ...
    if (!followData[username]) {
      followData[username] = { isFollowing: false };
    }
  ...
  useEffect(() => {
    ...
    const loadFollowerData = () => {
      if (username !== 'user') {
        ...
        const followData = JSON.parse(localStorage.getItem('followData') || '{}');
        const userFollowData = followData[username] || { isFollowing: false };
          ...
          <ProfileCard>
            ...
            <ProfileStats>
              ...
              <StatItem $clickable onClick={handleFollowerClick}>
                <StatNumber>{username !== 'user' ? (user?.followers || 0) : 0}</StatNumber>
                  ...
                  {savedSchedules.length > 0 ? (
                    savedSchedules.map(trip => (
                      <SavedScheduleCard
                        ...
                        onDelete={() => deleteSavedSchedule(trip.id)}
                      />
                    ))
                  ) : (
                    ...
                    {myCompanionPosts.length > 0 ? (
                      myCompanionPosts.map((post) => (
                        <PostListItem
                          ...
                          onEdit={() => editCompanionPost(post.id)}
                          onDelete={() => deleteCompanionPost(post.id)}
                        />
                      ))
                    ) : (
                    ...
                    {myTravelSchedules.length > 0 ? (
                      myTravelSchedules.map((schedule) => (
                        <PostListItem
                          ...
                          onEdit={() => editTravelSchedule(schedule.id)}
                          onDelete={() => deleteTravelSchedule(schedule.id)}
                        />
                      ))
                    ) : (
